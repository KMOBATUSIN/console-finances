...
function analysis(finances) {
  ...
  let totalMoney = 0;
  let totalChange = 0;
  let increase = { date: "", amount: 0 };
  let decrease = { date: "", amount: 0 };

  for (let x = 0; x < totalMonths; x++) {
    /*[x][0] means from the  current array  'x' pull the 1st value which is the date
    [x][1] means from the current array  'x' pull the 2nd value which is the money*/
  let currentDate = finances[x][0];
  let currentAmount = finances[x][1];
  ...
  if (x > 0) {
    //[x-1][1] means from the previous month array pull the second value
    let change = currentAmount - finances[x - 1][1];
    totalChange += change;

    
    if (change > increase.amount) {
      increase.date = currentDate;
      increase.amount = change;
    } else if (change < decrease.amount) {
      decrease.date = currentDate;
